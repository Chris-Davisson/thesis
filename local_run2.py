...
Usage:
    ...
    python local_run2.py --model "Qwen/Qwen2.5-72B-Instruct" --guided
    python local_run2.py --model "..." --concurrency 32 --port 8001
    python local_run2.py --model "..." --scan-ids 1 2 3  # specific scans only
    python local_run2.py --model "..." --temperature 0.7 --top-p 0.95 --seed 1
"""
...
def write_result_to_db(
    ...
    db.model_runs.insert_one({
        ...
        "model": {
            ...
            "guided_decoding": model_config["guided_decoding"],
        },
        "messages": messages,
...
async def async_main(args):
    ...
    # Get scans — pull the raw nmap XML, no parsing
    projection = {"_id": 1, "nmap.xml": 1}
    if args.scan_ids:
        scans = list(db.scans.find({"_id": {"$in": args.scan_ids}}, projection))
        missing = set(args.scan_ids) - {s["_id"] for s in scans}
        if missing:
            print(f"Warning: scan IDs not found: {sorted(missing)}")
    else:
        scans = list(db.scans.find({}, projection).sort("_id"))
    ...
    skipped = 0
    for scan in scans:
        xml_payload = (scan.get("nmap") or {}).get("xml")
        if not xml_payload:
            print(f"  SKIP  scan {scan['_id']} has no nmap.xml")
            skipped += 1
            continue
    ...
    n_scans = len(scans) - skipped
    n_prompts = len(prompts)
    n_tasks = len(tasks)

    print(f"\nLocal vLLM sweep configuration (raw XML payload):")
    print(f"  Note:        {RUN_NOTE}")
    ...
    print(f"  Scans:       {n_scans}" + (f" ({skipped} skipped — no XML)" if skipped else ""))
    ...
    print(f"All runs tagged with note={RUN_NOTE!r}, payload_kind='nmap_xml'.")
